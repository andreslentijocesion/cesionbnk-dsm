        ...
        return (
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div className={cn(
                "size-8 rounded-full flex items-center justify-center border-2 transition-all text-xs font-semibold",
                done   ? "border-primary bg-primary text-primary-foreground" :
                active ? "border-primary bg-background text-primary" :
                         "border-muted-foreground/25 bg-muted text-muted-foreground"
              )}>
                {done ? <CheckCircle2 className="size-4" /> : <Icon className="size-3.5 .5" />}
              </div>
              <span className={cn(
  ...
  return (
    <div className="space-y-1.5">
      ...
      {React.isValidElement(children)
        ...
        : children}
      {hint && !error && <p className="text-xs text-muted-foreground flex items-center gap-1"><Info className="size-3" />{hint}</p>}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-warning/40 bg-warning/5 px-4 py-3 flex gap-2.5">
        <Info className="size-4 text-warning flex-shrink-0 mt-0.5" />
      ...
      <div className="px-6 py-4 flex items-center justify-between bg-muted/30">
        ...
        {step < 3 ? (
          ...
          <Button size="sm" onClick={() => setSubmitted(true)}>
            <CheckCircle2 className="size-4 mr-1.5" /> Confirmar y radicar
          </Button>
        )}
      </div>
    </Card>
