...
function QueueKpis({ data }: { data: QueueRecord[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {[
        { label: "En cola",         value: `${total} ops`,     icon: <Clock className="h-4 w-4 text-primary" />,      bg: "bg-primary/10" },
        { label: "Valor total",     value: new Intl.NumberFormat("es-CO", { notation: "compact", style: "currency", currency: "COP", maximumFractionDigits: 1 }).format(totalValue), icon: <DollarSign className="h-4 w-4 text-secondary" />, bg: "bg-secondary/10" },
        { label: "Prioridad alta",  value: `${altas} ops`,     icon: <AlertTriangle className="h-4 w-4 text-destructive" />, bg: "bg-destructive/10" },
        { label: "Días prom. cola", value: `${avgDays} días`,  icon: <FileText className="h-4 w-4 text-muted-foreground" />, bg: "bg-muted" },
      ].map((k) => (
              ...
              {selected.size > 0 && (
                <>
                  <Button size="sm" className="h-9 bg-success hover:bg-success/90" onClick={() => decide([...selected], "approved")}>
                    <CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Aprobar ({selected.size})
                  </Button>
                  <Button size="sm" variant="destructive" className="h-9" onClick={() => decide([...selected], "rejected")}>
                    <XCircle className="h-3.5 w-3.5 mr-1" /> Rechazar ({selected.size})
                  </Button>
                </>
              )}
            </div>
