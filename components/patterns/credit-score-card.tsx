...
function ScoreArc({ score }: { score: number }) {
  ...
  return (
    <svg width="140" height="90" viewBox="0 0 140 90">
      ...
      />
      {/* Score text */}
      <text x={cx} y={cy + 2} textAnchor="middle" style={{ fontSize: 22, fontWeight: 700, fill: "var(--foreground)" }}>
        {score}
      </text>
      <text x={cx} y={cy + 18} textAnchor="middle" style={{ fontSize: 10, fill: "var(--muted-foreground)" }}>
        de 1000
      </text>
    </svg>
  );
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className={cn("flex items-center gap-1.5 rounded-lg px-2.5 py-1.5", cfg.bg)}>
            <RiskIcon className={cn("h-4 w-4", cfg.color)} />
              ...
              return (
                <div key={m.label} className="rounded-md bg-muted/40 px-3 py-2">
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    ...
                    {m.trend && (
                      <TIcon className={cn("h-3 w-3", trendColor(m.trend))} />
                    )}
                  </div>
                </div>
              );
