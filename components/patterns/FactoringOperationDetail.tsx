  ...
  return [
    {
      ...
      icon: <FileText className="size-3.5 .5" />,
      done: true,
    },
    {
      ...
      icon: <Stamp className="size-3.5 .5" />,
    ...
    },
    {
      ...
      icon: <DollarSign className="size-3.5 .5" />,
    ...
    },
    {
      ...
      icon: <Clock className="size-3.5 .5" />,
      done: done("en-cobro"),
    },
    {
      label: "Cobro total",
      ...
      icon: <CheckCircle2 className="size-3.5 .5" />,
...
function DetailActions({ status }: { status: FactoringStatus }) {
  const sets: Record<FactoringStatus, React.ReactNode> = {
    aprobado: (
      <>
        <Button size="sm" className="flex-1"><DollarSign className="size-4 mr-1.5" />Desembolsar</Button>
        <Button size="sm" variant="destructive" className="flex-1"><XCircle className="size-4 mr-1.5" />Rechazar</Button>
      </>
    ),
    desembolsado: (
      <>
        <Button size="sm" className="flex-1"><CheckCircle2 className="size-4 mr-1.5" />Marcar cobrado</Button>
        <Button size="sm" variant="outline" className="flex-1"><Send className="size-4 mr-1.5" />Notificar deudor</Button>
      </>
    ),
    "en-cobro": (
      <>
        <Button size="sm" className="flex-1"><CheckCircle2 className="size-4 mr-1.5" />Registrar cobro</Button>
        <Button size="sm" variant="outline" className="flex-1"><Send className="size-4 mr-1.5" />Enviar recordatorio</Button>
      </>
    ),
    cobrado: (
      <Button size="sm" variant="outline" className="flex-1"><Download className="size-4 mr-1.5" />Descargar soporte</Button>
    ),
    vencido: (
      <>
        <Button size="sm" className="flex-1"><RotateCcw className="size-4 mr-1.5" />Iniciar cobranza</Button>
        <Button size="sm" variant="outline" className="flex-1"><CheckCircle2 className="size-4 mr-1.5" />Marcar cobrado</Button>
      </>
    ),
    rechazado: (
      <Button size="sm" variant="outline" className="flex-1"><RotateCcw className="size-4 mr-1.5" />Reactivar</Button>
    ),
  };
              ...
              <div className="relative space-y-0">
                {timeline.map((ev, i) => (
                  <div key={i} className="flex gap-4 pb-6 last:pb-0 relative">
                    ...
                    {/* Icon */}
                    <div className={cn(
                      "size-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 z-10",
                      ev.done
                    ...
                    <div className="pt-0.5 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {ev.done && <CheckCircle2 className="size-3 text-success flex-shrink-0" />}
                      </div>
                ...
                <div key={doc.name} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/40 transition-colors group">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" className="size-7">
                      <Download className="size-3.5 .5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="size-7">
                      <ExternalLink className="size-3.5 .5" />
                    </Button>
                  </div>
                </div>
              ))}
              <Button variant="outline" size="sm" className="w-full mt-2 text-xs border-dashed">
                <FileText className="size-3.5 .5 mr-1.5" /> Adjuntar documento
              </Button>
            </TabsContent>
          </Tabs>
        </div>
