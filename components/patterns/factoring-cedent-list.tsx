  ...
  return (
    <TableHead className={cn("cursor-pointer select-none whitespace-nowrap text-xs", className)} onClick={() => onSort(col)}>
      <span className="inline-flex items-center gap-1 group">
        ...
        {!active && <ArrowUpDown className="size-3 text-muted-foreground/40 group-hover:text-muted-foreground transition-colors" />}
      </span>
    </TableHead>
  );
}
...
export function FactoringCedentList() {
  ...
  return (
    <MasterDataGrid
      ...
      onFilterChange={(_: string, v: string) => { setStatusFilter(v); setPage(1); }}
      onResetFilters={() => { setStatusFilter("all"); setSearch(""); setPage(1); }}
      newActionLabel="Nuevo cedente"
      onNewAction={() => {}}
      newActionIcon={<Building2 className="size-4" />}
      ...
      toolbarActions={
        <div className="hidden sm:flex items-center gap-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><CheckCircle2 className="size-3 text-success" />{activos} activos</span>
          <span className="flex items-center gap-1"><AlertTriangle className="size-3 text-warning" />{enRevision} revisión</span>
          <span className="flex items-center gap-1"><AlertTriangle className="size-3 text-destructive" />{suspendidos} suspendidos</span>
        </div>
      }
    >
