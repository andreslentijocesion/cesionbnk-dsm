...
export function StatusKPICard({
  ...
  const getIcon = () => {
    switch (variant) {
      case "negotiation": return <Clock className="size-4 text-primary" />;
      case "disbursed": return <CheckCircle className="size-4 text-success-on-subtle" />;
      case "warning": return <TrendingUp className="size-4 text-warning-on-subtle" />;
      case "error": return <TrendingUp className="size-4 text-destructive" />;
  ...
  const getAmountColor = () => {
    switch (variant) {
      ...
      case "disbursed": return "text-success-on-subtle";
      case "warning": return "text-warning-on-subtle";
      ...
      <CardContent className="p-5 flex flex-col justify-between h-full space-y-4">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <h3 className="font-semibold text-sm text-foreground/80 flex items-center gap-2">{title}</h3>
        ...
        <div className="space-y-1">
          {amount && <div className={cn("text-2xl font-bold tracking-tight", getAmountColor())}>{amount}</div>}
          {count !== undefined && (
