...
function Avatar({ initials, className }: { initials: string; className?: string }) {
  return (
    <div className={cn(
      "flex-shrink-0 size-7 rounded-full bg-primary/10 flex items-center justify-center text-2xs font-bold text-primary",
      className
    )}>
      {initials}
    </div>
          ...
          <div className="flex gap-2 items-start">
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="flex items-center justify-between gap-2">
                {allowPublic ? (
                  <div className="flex items-center rounded-lg border border-border bg-muted p-0.5 gap-0.5">
                    {(["internal", "public"] as CommentVisibility[]).map((v) => (
                      ...
                      >
                        {v === "internal" ? <Lock className="size-3" /> : <Globe className="size-3" />}
                ...
                >
                  <Send className="size-3.5 .5 mr-1.5" />
                  Comentar
                </Button>
              </div>
            </div>
...
function CommentItem({
  ...
  return (
    <div className="flex gap-2.5 group">
      <Avatar initials={comment.initials} className={isReply ? "size-6 text-2xs" : undefined} />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className="text-xs font-semibold text-foreground">{comment.author}</span>
          {comment.visibility === "internal" && (
            <Badge variant="outline" className="text-xs">
              <Lock className="size-2.5 .5" /> Interno
            </Badge>
          )}
