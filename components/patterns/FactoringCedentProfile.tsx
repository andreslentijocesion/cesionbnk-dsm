        ...
        <CardContent className="p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-4">
              <div className="size-14 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 border border-primary/20">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                  <span className="flex items-center gap-1"><Shield className="size-3" /> NIT {cedent.nit}</span>
                  ...
                  <span className="flex items-center gap-1"><Star className="size-3" /> Analista: {cedent.analista}</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <Button variant="outline" size="sm" className="h-8 text-xs">
                <Edit className="size-3.5 .5 mr-1.5" /> Editar
              </Button>
              <Button size="sm" className="h-8 text-xs">
                <FileText className="size-3.5 .5 mr-1.5" /> Nueva operación
              </Button>
            </div>
          </div>
              ...
              <CardContent className="px-2 pb-3">
                <SafeChartContainer width="100%" height={180} minHeight="180px">
                  <BarChart data={historialMensual} margin={{ top: 5, right: 16, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="mes" tick={{ fontSize: "0.6875rem", fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: "0.6875rem", fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v}M`} width={48} />
                    <Tooltip formatter={(v: number) => [`$${v}M`, ""]} contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: "var(--radius)", fontSize: "0.75rem" }} />
            ...
            <div className="space-y-4">
                ...
                <CardContent className="px-4 pb-3 space-y-3">
                  ...
                  <SafeChartContainer width="100%" height={60} minHeight="60px">
                    <LineChart data={scoreHistory}>
                      ...
                      <Tooltip formatter={(v: number) => [v, "Score"]} contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: "var(--radius)", fontSize: "0.6875rem" }} />
                    </LineChart>
                  </SafeChartContainer>
              ...
              <Card>
                ...
                <CardContent className="px-4 pb-3">
                  <InfoRow label="Responsable" value={cedent.contacto} icon={<Building2 className="size-3.5 .5" />} />
                  <InfoRow label="Teléfono" value={cedent.telefono} icon={<Phone className="size-3.5 .5" />} />
                  <InfoRow label="Email" value={cedent.email} icon={<Mail className="size-3.5 .5" />} />
                  <InfoRow label="Dirección" value={cedent.direccion} icon={<MapPin className="size-3.5 .5" />} />
                </CardContent>
              </Card>
            </div>
          </div>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="px-4 pt-4 pb-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="h-8 text-xs">
                  <Download className="size-3.5 .5 mr-1.5" /> Exportar
                </Button>
              </div>
            </CardHeader>
            ...
            <CardHeader className="px-4 pt-4 pb-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="h-8 text-xs border-dashed">
                  <FileText className="size-3.5 .5 mr-1.5" /> Subir documento
                </Button>
              </div>
            </CardHeader>
            <CardContent className="p-4 space-y-2">
              {documentos.map((doc) => (
                <div key={doc.nombre} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/30 transition-colors group">
                  <div className={cn("size-8 rounded-md flex items-center justify-center flex-shrink-0", doc.vigente ? "bg-primary/10" : "bg-destructive/10")}>
                    <FileText className={cn("size-4", doc.vigente ? "text-primary" : "text-destructive")} />
                  </div>
                  <div className="flex-1 min-w-0">
