...
function FieldTypeItem({ type, label, icon: Icon }: { type: string; label: string; icon: React.ComponentType<{ className?: string }> }) {
  ...
  return (
    <div ref={drag} className={cn("flex items-center gap-3 p-3 border rounded-lg cursor-move transition-all hover:border-primary hover:bg-accent", isDragging && "opacity-50")}>
      ...
      <div className="flex-1 space-y-3">
        ...
        {field.type === "rating" && (
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map((star) => <Star key={star} className="size-5 text-muted-foreground" />)}
          </div>
        )}
          ...
          <div className="space-y-2">
            {field.options?.map((option, optIndex) => (
              <div key={optIndex} className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); updateField(field.id, { options: field.options?.filter((_, i) => i !== optIndex) }); }} className="size-8 p-0">
...
function FormCanvas({ onDrop, fields, moveField, deleteField, updateField }: {
  ...
  return (
    <div ref={drop} className={cn("min-h-[400px] rounded-lg transition-all p-4", isOver && "bg-muted ring-2 ring-primary ring-inset")}>
      {fields.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[380px] border-2 border-dashed rounded-lg border-muted">
          <Plus className="size-12 mb-4 text-muted-foreground" />
                    ...
                    {fields.map((field) => {
                      ...
                      return (
                        <div key={field.id} className={cn("space-y-2", colSpanClass)}>
                          ...
                          {field.type === "rating" && <div className="flex items-center gap-1">{[1,2,3,4,5].map((s) => <Star key={s} className="size-5 cursor-pointer text-muted-foreground" />)}</div>}
                          {(field.type === "checkbox" || field.type === "radio") && (
                            <div className="space-y-2">
                              {field.options?.map((option, i) => <div key={i} className="flex items-center gap-2"><input type={field.type === "checkbox" ? "checkbox" : "radio"} className="size-4" name={field.id} /><span className="text-sm">{option}</span></div>)}
                            </div>
                          )}
                          {field.type === "select" && (
