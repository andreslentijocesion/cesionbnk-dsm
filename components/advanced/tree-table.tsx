                    ...
                    <TableRow className={cn("bg-background hover:bg-accent transition-colors", checkState === true && "bg-primary/10 hover:bg-primary/15", isMatch && "bg-yellow-50 dark:bg-yellow-900")} data-state={checkState === true ? "selected" : undefined}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:text-destructive" onClick={() => onCancelOperation?.(op)} disabled={op.status === "canceled" || op.status === "disbursed"}><Ban className="h-4 w-4" /><span className="sr-only">Cancel</span></Button></TooltipTrigger><TooltipContent>Cancel operation</TooltipContent></Tooltip>
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => onDownloadExcel?.(op)}><FileSpreadsheet className="h-4 w-4" /><span className="sr-only">Excel</span></Button></TooltipTrigger><TooltipContent>Download Excel</TooltipContent></Tooltip>
                        </div>
                      </TableCell>
                    </TableRow>
                        ...
                        <TableRow key={`payer-${group.groupId}`} className={cn("bg-accent hover:bg-muted transition-colors", payerCheckState === true && "bg-primary/10 hover:bg-primary/15", payerMatch && "bg-yellow-50 dark:bg-yellow-900")}>
                          <TableCell>
                            <div className="flex items-center gap-1 pl-4">
                              ...
                              <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-primary/10" onClick={() => toggleExpandPayer(group.groupId)}>
                                {isPayerExpanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
                              </Button>
                            </div>
                          </TableCell>
                          <TableCell>
