          ...
          return (
            <div key={sig.id} className={cn("px-5 py-3.5", cfg.row, isBlocked && "opacity-50")}>
              <div className="flex items-center gap-3">
                {/* Order / status circle */}
                <div className={cn(
                  "h-8 w-8 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-bold",
                  statusIcon[sig.status],
                )}>
                  {sig.status === "signed"   ? <Check className="h-3.5 w-3.5" /> :
                   sig.status === "rejected" ? <X className="h-3.5 w-3.5" /> :
